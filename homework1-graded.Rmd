...
**JC Grading: -1**
Your coding is sound, but notice something funny that dev1==dev2 reports false.  Really though they are the same except for very small rounding error (see below).
...
```{r}
men<-x[ which(gender=="M"),]
men
women<-x[which(gender=='F'),]
women
```
...
```{r}
t.test(women$los,men$los,var.equal=F)
```
